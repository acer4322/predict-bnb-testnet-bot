...
def patch_tests(text: str) -> str:
    # feature/regime-guard intentionally records that the reverse shadow
    # depends on a source trade from the same market. Some branch revisions
    # still contain the older expected diagnostic string.
    stale_dependency = '"open_only_after_R_FUTURES_LEAD_trade"'
    current_dependency = (
        '"open_only_after_same_market_R_FUTURES_LEAD_trade"'
    )
    if stale_dependency in text:
        text = text.replace(
            stale_dependency,
            current_dependency,
            1,
