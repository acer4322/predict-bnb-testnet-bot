...
try {
    ...
    $Settlement = Join-Path $Root "data\simulation.db"
    ...
    if (-not (Test-Path $Settlement)) {
        throw "Missing settlement DB: $Settlement"
    }
    ...
    Write-Host "`n[1/3] Validate paper PnL analyzer..."
    python -m py_compile .\tools\analyze_target_taker_ordinary_paper_pnl_v1.py
    if ($LASTEXITCODE -ne 0) { throw "Paper PnL syntax check failed." }

    if ($RunTests) {
        Write-Host "`n[2/3] Run strict-as-of / official-settlement / fee tests..."
        python -m pytest -q .\tests\test_target_taker_ordinary_paper_pnl_v1.py
    ...
    Write-Host "`n[3/3] Replay OOF actions against historical asks and OFFICIAL outcomes..."
    $Args = @(
    ...
    Write-Host "Report: data\research\target_taker_ordinary_paper_pnl_v1_report.json"
}
finally {
    Pop-Location
}
