...
def _start_cross_oracle() -> subprocess.Popen[bytes] | None:
    ...
    print(
        ...
        "with hard rollover invalidation, hardened Gamma discovery and restricted public-readonly HTTP/WSS TLS fallback",
        flush=True,
    )
    return subprocess.Popen(
        [sys.executable, "-m", "predict_bot.cross_oracle_rollover_guard"]
    )
...
def _start_poly_gap_live() -> subprocess.Popen[bytes] | None:
    ...
    print(
        ...
        "V9 on port 8769 (10% entry cap, 20% exit tolerance, order reconciliation)",
        flush=True,
    )
    return subprocess.Popen(
        [sys.executable, "-m", "predict_bot.poly_gap_live_v9"]
    )
