...
def _wallet_shadow_process() -> subprocess.Popen[bytes] | None:
    ...
    print(
        f"multi-asset live: starting read-only BTC wallet shadow observer V4.1 on {WALLET_SHADOW_PORT}; "
        f"target={target}; retention={retention}d; Taker V0+V1 paper A/B; no order-write path",
        flush=True,
    )
    return subprocess.Popen([sys.executable, "-m", "predict_bot.predict_wallet_shadow_observer_v4_1"])
