...
if ($ExistingPid) {
    ...
    Write-Host "Replacing old Poly Fast Signal PID=$ExistingPid with V17 lifecycle-authoritative pending-latch fix entry=$EntryMode."
...
$Process = Start-Process -FilePath "python" -ArgumentList @("-m", "predict_bot.poly_fast_signal_v17") -WorkingDirectory $Root -WindowStyle Hidden -RedirectStandardOutput $Stdout -RedirectStandardError $Stderr -PassThru
...
if (-not ([string]$State.state.version).Contains("POLY_FAST_SIGNAL_V17")) {
    throw "Unexpected Poly Fast version: $($State.state.version)"
}
if (-not [bool]$State.state.architecture.pendingEntryLifecycleAuthority) {
    throw "V17 pending-entry lifecycle authority fix is not active."
}
if (-not [bool]$State.state.architecture.staleSubmittedLocalLatchCannotBlockFutureRounds) {
    throw "V17 stale SUBMITTED local-latch release invariant is not active."
}
Write-Host "Poly Fast Signal V17 is ready: http://127.0.0.1:$Port/state"
...
Write-Host "  Pending    : local queued-entry latch yields to 8781 durable /poly-lifecycle once the round is adopted"
Write-Host "  Old fill   : historical SUBMITTED intent cannot permanently fence later rounds after 8781 reports no active round"
Write-Host "  Safe retry : REJECTED still requires no venue-write evidence + >=1s cooldown + newer Binance book"
Write-Host "  Never retry: AMBIGUOUS/uncertain venue-write states remain fail-closed"
...
Write-Host "  Lifecycle  : one active round per asset; same-direction repeats ignored while OPEN; 8781 is active-round authority"
