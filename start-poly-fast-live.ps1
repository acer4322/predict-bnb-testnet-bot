...
if ($ExistingPid) {
    ...
    Write-Host "Replacing old Poly Fast Signal PID=$ExistingPid with V12 entry=$EntryMode."
...
$Process = Start-Process -FilePath "python" -ArgumentList @("-m", "predict_bot.poly_fast_signal_v12") -WorkingDirectory $Root -WindowStyle Hidden -RedirectStandardOutput $Stdout -RedirectStandardError $Stderr -PassThru
...
if (-not ([string]$State.state.version).Contains("POLY_FAST_SIGNAL_V12")) {
    throw "Unexpected Poly Fast version: $($State.state.version)"
}
Write-Host "Poly Fast Signal V12 is ready: http://127.0.0.1:$Port/state"
...
Write-Host "  Diagnostic : postRejectRearm records evaluator states after safe rearm; observation-only"
