...
- 正式實單頁可選擇 M 系列與 live 白名單研究訊號、設定每策略金額，並調整 M0 每小時門檻。`R_MICROPRICE` 與 `R_OFI` 已列入 live 白名單；最多四個實單策略槽位各自持久化金額，其中前三格可設定 Observer 開關與 `F1 / V2 / V3 / V4 / V6` 版本，第四格固定不使用 Observer。每個策略槽位也可獨立開啟「兩連敗後冷卻一個市場」；它只使用該策略的新正式結算，第二次連敗後跳過下一個原本合格的市場並歸零，不採用被跳過市場的事後結果。此選項預設關閉且舊設定不會自動啟用。版本可在 Observer 關閉時預先選擇；啟用後只過濾該槽位的後續新訊號，缺少同市場 READY context 時 fail closed。頁面會先從輕量 `/api/live-rules` 載入真實規則，不再把預設 `M0W` 誤標為已同步。舊版共用 Observer 設定會在載入時遷移到每策略設定。選擇 `M01O_F1` 時仍使用原本的 F1 gate。設定持久化、只影響新訊號；主機本身與同一私有區域網路中經合法監察頁連線的手機都能儲存。手機仍不能恢復已暫停的實單。
- 最上方固定顯示目前市場的實單持倉區；沒有成交時顯示空持倉。任一正式實單策略有已成交且未結算的持倉時，市場卡會用每秒更新的 outcome 最佳買價估算目前可售價值、收益與 ROI。使用者可二次確認後選擇「現價 LIMIT GTC」或「MARKET FOK」賣出；後端會重新核對 Binance wallet shares、取得 SELL quote 並將出場獨立記帳。手動賣單確認完整成交後，策略會立即按精確 SELL quote 回款與原始成交成本封帳，顯示 `WIN／LOSS · 手動平倉`，不再等待市場官方結算。手動賣出可由本機或同一 private LAN 上、透過本機 `4310` 面板網址開啟的裝置操作；API 要求相符 Origin／Host 與專用確認標頭，Windows 防火牆仍限 `LocalSubnet`，公網或不相符來源一律拒絕。timeout／5xx 會標成不確定且不允許盲目重送。
...
`R_MICROPRICE`、`R_OFI`、`R_FUTURES_LEAD`、`R_CALIBRATED_VALUE`、`R_CONSENSUS` 是五組主策略；另有主研究頁 Shadow、五組 Lead Observer Shadow（`R_FUTURES_LEAD_OBSERVER_F1/V2/V3/V4/V6`），以及固定的 `R_OFI_OBSERVER_V3`、`R_MICROPRICE_OBSERVER_V3/V6`、`R_CALIBRATED_VALUE_OBSERVER_V6`。`R_MICROPRICE_OBSERVER_AUTO_V6` 與 `R_CALIBRATED_VALUE_OBSERVER_AUTO_V6` 使用當前市場以前的官方來源結算，以最近 30 筆快速窗和 100 筆慢窗決定套用或 bypass V6；只有兩窗都顯示通過組獲利且阻擋組虧損、allow/block cohort 皆達最低樣本時才套用 V6。兩組 AUTO 都是 paper-only，資料不足時 bypass，且不在 live 轉送白名單。`R_CALIBRATED_VALUE_CONTINUOUS_V2` 與 `R_FUTURES_LEAD_CONTINUOUS_V2` 同樣只依賴先前官方結算，使用最近 200 筆來源結果做固定分箱與 Beta 收縮。所有 Observer 只能在同市場來源策略已實際開出模擬單後，以相同方向與相同可執行成交模型進場。
...
PAIR 實單準備不使用共用單邊策略的「策略可用簿年齡」。它會重新讀取最新獨立雙 Token REST 快照，以 REST 收包年齡判斷本機資料是否仍即時，並另外套用上述 exchange content age 與雙簿時差設定。`PAIR_ARB_010` 的設定金額只是每市場總上限；正式報價前會沿兩邊最新多層 Ask 同步走簿，只保留邊際含費淨優勢仍達 0.01 的共同份額，必要時自動把兩腿縮到當下仍有正收益的深度，不會為了吃滿設定金額掃入無利潤價位。每腿 signed quote 可在原訊號價上方探索最多 `0.10`，但不會只因單腿過價就送單：若兩腿份額差超過 0.25% 會先嘗試等量重報，最後必須以較少的一腿份額計算保證回款，扣除兩腿報價成本、provider fee 與 0.01 USDT 緩衝後，仍鎖定至少每份 0.01 USDT 淨利才可併行送出兩腿。容量、到期時間或任一整組條件未通過時兩腿皆不送單。
...
`/api/state` 的 optional `mRealtime` 狀態列顯示 `status`、`mode`、`paperOnly`、`marketId`、`eventRate`、`queueDepth`、`processedEvents`、`droppedEvents`、`evaluations`、`lastEventAt`、`lastDecisionAt`、`lastQueueDelayMs`、`lastDecisionDurationMs`、`spotAgeMs`、`spotTradeIngressAgeMs`、`spotTradeProcessedAgeMs`、`spotPriceSource`、`futuresAgeMs`、`predictionBookAgeMs`、`predictionStrategyBookAgeMs`、`predictionRestReceiptAgeMs`、`predictionExchangeContentAgeMs`、`schedulerTickMs`、`m7EventReorderGraceMs`、資料完整性與 replay/drop 診斷，以及 `error`。`spotTradeIngressAgeMs` 是 trade WebSocket callback 到現在的年齡；`spotTradeProcessedAgeMs` 是最後一筆已由 M worker 處理的 trade 資料年齡。價格型策略先使用 Spot trade，trade 無法提供不超過 4 秒的資料時，才使用獨立 Spot book socket 的 `bookTicker` midpoint；不使用 aggTrade 或不受控的最新值快取作為預設補救。Prediction 三種年齡分別代表最後一份通過安全閘門的策略可用簿、最近 REST 快照抵達本機至今，以及該最新 REST 快照的交易所內容版本至今；被閘門拒絕的快照只更新後兩項遙測，不會取代策略可用簿。`predictionBookAgeMs` 保留為策略可用簿年齡的相容別名。M7 目前保留 10 ms 的本機跨 socket 排序寬限，避免 deadline 前已 timestamp、但晚數毫秒排入 worker 的 Spot trade 被漏掉；這個寬限會如實記入 scheduler lateness。後端按事件驅動 M 系列判斷；網站每 1 秒刷新狀態列和帳本，不會改變後端事件處理頻率。
