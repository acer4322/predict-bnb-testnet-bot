...
export default function FuturesLeadConfirmationAddControl() {
  ...
  useEffect(() => {
    const locate = () => {
      ...
      document.querySelectorAll<HTMLSelectElement>(
        ...
        const label = modeSelect.getAttribute("aria-label") ?? "";
        const match = label.match(/實單策略\s+(\d+)\s+資金模式/);
        if (!match) return;
        const strategySelect = document.querySelector<HTMLSelectElement>(
          `select[aria-label="實單策略 ${match[1]}"]`,
        );
        const confirmationOption = modeSelect.querySelector<HTMLOptionElement>(
          'option[value="CONFIRMATION_ADD"]',
        );
        if (!strategySelect || !confirmationOption) return;
        if (strategySelect.value === "M01O_F1") {
          confirmationOption.disabled = true;
          confirmationOption.textContent = "順勢確認加碼（F1 已移除）";
        } else if (strategySelect.value === "R_FUTURES_LEAD") {
          confirmationOption.disabled = true;
          confirmationOption.textContent = "順勢確認加碼（使用下方 Lead 控制）";
        }
      });
    };
  ...
  const changeSlot = (
    ...
    if (!draft) return;
    setDraft(updateLeadSlot(draft, index, mode, initial, add));
    setDirty(true);
  ...
  const save = async () => {
    ...
    const summary = leadSlots.map(({ index }) => {
      ...
      return mode === "CONFIRMATION_ADD"
        ? `槽位 ${index + 1}：初始 ${initial} USDT，1.1×～1.4× 每階 ${add} USDT，總上限 ${initial + add * 4} USDT`
    ...
    try {
      ...
      setState(payload.liveM0W ?? state);
      setDraft(payload.liveM0W?.rules ? copyRules(payload.liveM0W.rules) : draft);
      setDirty(false);
  ...
  const min = state?.configurableStakeRangeUsdt?.min ?? 0.01;
  const max = state?.configurableStakeRangeUsdt?.max ?? 100;
  const supported = state?.policy?.confirmationAdd?.supportedSources ?? [];
  ...
  return createPortal(
    <section style={{
      ...
      <div style={{ display: "flex", justifyContent: "space-between", gap: 16, alignItems: "flex-start", flexWrap: "wrap" }}>
        ...
        <span className={supported.includes("R_FUTURES_LEAD") ? "m-exit-api-state live" : "m-exit-api-state"}>
          {supported.includes("R_FUTURES_LEAD") ? "後端已支援" : "等待重啟載入新版後端"}
        </span>
      </div>

      {leadSlots.length === 0 ? <p style={{ margin: "16px 0 0" }}>
        目前四個實單槽位沒有選擇 R_FUTURES_LEAD；先在上方選定該策略，這裡便會出現資金模式。
        ...
        return <div key={index} style={{
          display: "grid",
          gridTemplateColumns: "minmax(220px, 1fr) minmax(150px, .7fr) minmax(150px, .7fr) minmax(160px, .8fr)",
          gap: 12,
          marginTop: 16,
          alignItems: "end",
        }}>
          <label><span>實單槽位 {index + 1} 資金模式</span><select value={mode} onChange={event => changeSlot(index, event.target.value as ExecutionMode, initial, add)}><option value="FIXED">固定一次下單</option><option value="CONFIRMATION_ADD">順勢確認加碼</option></select></label>
          <label><span>初始 USDT</span><input type="number" min={min} max={max} step="0.01" value={initial} onChange={event => changeSlot(index, mode, Number(event.target.value), add)} /></label>
          <label><span>每階 USDT</span><input type="number" min={min} max={max} step="0.01" disabled={mode !== "CONFIRMATION_ADD"} value={add} onChange={event => changeSlot(index, mode, initial, Number(event.target.value))} /></label>
          <div><span>單市場總上限</span><strong style={{ display: "block", marginTop: 8 }}>{total.toFixed(2)} USDT</strong><small>{mode === "CONFIRMATION_ADD" ? "初始＋四次加碼" : "單次固定下單"}</small></div>
        </div>;
      })}

      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, marginTop: 16, flexWrap: "wrap" }}>
        <small>{status}</small>
        <button type="button" disabled={!dirty || saving || leadSlots.length === 0} onClick={save}>
