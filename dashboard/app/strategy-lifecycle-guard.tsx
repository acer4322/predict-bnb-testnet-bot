...
export default function StrategyLifecycleGuardPanel({ data }: { data?: StrategyLifecycleState | null }) {
  ...
  return <section className="live-hourly-guard allow" aria-label="Strategy Lifecycle Guard">
    <div className="live-hourly-guard-head">
      <div>
        <span className="eyebrow">STRATEGY LIFECYCLE GUARD · REAL LIVE SETTLEMENTS</span>
    ...
    <p>
      V1 只提供決策證據，不自動暫停、不自動換策略、也不修改 stake。Last20／Last50 只使用該策略自己的真實已結算實單；
      CONFIRM_ADD 子單與 PAIR 雙腿先合併成同一策略、同一市場結果，避免多腿執行把樣本數灌大。
    </p>
    ...
    <div className="table-scroll">
      <table>
        ...
        <tbody>
          {rows.length === 0
            ? <tr><td colSpan={7} className="empty">等待實單 Lifecycle payload。</td></tr>
            : rows.map(row => {
    ...
    <p>
      判級：樣本 &lt;20 為 BUILDING；20–49 筆只用 Last20 判斷 WATCH；50 筆以上若 Last20、Last50 同負為 DEGRADED，
      Last20 轉正但 Last50 仍負為 RECOVERY，Last20 負而 Last50 正為 WATCH，兩窗皆非負才為 ACTIVE。這些狀態目前完全不進入下單 hot path。
    </p>
  </section>;
}
