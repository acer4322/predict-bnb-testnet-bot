...
Push-Location $Root
try {
    Write-Host "TARGET MAKER -> TAKER INVENTORY LIFECYCLE V1.1"
    ...
    Write-Host "Action unit: exactly one row per reconstructed order-hash parent; duplicate same-ms completion rows are forbidden"
    ...
    Write-Host "`n[1/3] Validate lifecycle analyzers..."
    python -m py_compile .\tools\analyze_target_maker_taker_inventory_lifecycle_v1.py .\tools\analyze_target_maker_taker_inventory_lifecycle_v1_1.py
    if ($LASTEXITCODE -ne 0) { throw "Lifecycle analyzer syntax check failed." }

    if ($RunTests) {
        Write-Host "`n[2/3] Run signed-inventory / parent-dedupe tests..."
        python -m pytest -q .\tests\test_target_maker_taker_inventory_lifecycle_v1.py .\tests\test_target_maker_taker_inventory_lifecycle_v1_1.py
    ...
    Write-Host "`n[3/3] Replay official Maker/Taker inventory lifecycle with one action per Taker parent..."
    python .\tools\analyze_target_maker_taker_inventory_lifecycle_v1_1.py `
        --target-db $TargetDb `
    ...
    Write-Host "Invariant: classified Taker actions must equal unique derived Taker parents."
